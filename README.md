...
- **Xcode を先に起動**してから、メニュー **File → Open...** で `Husen.xcodeproj` を選んで開く
- または Xcode を起動し、出てきた「Open a project or file」で `husen` フォルダ内の `Husen.xcodeproj` を指定する
- Xcode が入っていない場合は、App Store から「Xcode」をインストールする（無料・容量大）

### 手順まとめ

1. リポジトリをクローンまたはダウンロードする
2. `Husen.xcodeproj` をダブルクリック（または Xcode の File → Open で開く）
